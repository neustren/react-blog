...
var port = parseInt(process.env.PORT) + 1 || 3101;
...
module.exports = {
  ...
  entry: {
    'main': [
      'webpack-hot-middleware/client?path=http://' + wphost + ':' + port + '/__webpack_hmr',
  ...
  output: {
    ...
    publicPath: 'http://' + wphost + ':' + port + '/dist/'
  },
  module: {
    loaders: [
      { test: /\.js$/, exclude: /node_modules/, loaders: ['babel?' + JSON.stringify(babelLoaderQuery)]}, //, 'eslint-loader'
      { test: /\.json$/, loader: 'json-loader' }, { test: /\.global\.css$/,
        loaders: [
          'style-loader',
          'css-loader?sourceMap'
        ]
      }, {
        test: /^((?!\.global).)*\.css$/, loader: 'style!css?modules&importLoaders=2&sourceMap&localIdentName=[local]___[hash:base64:5]!postcss?browsers=last 2 version' },
